...
<div class="help-section">
    ...
    <ul>
        ...
        <li>Each ZIP includes <code>manifest.json</code> (app id <code>SimpleCMS</code>, source dbname, schema snapshot),
            ...
            Page and post visual layouts are stored as <code>layout_json</code> in the SQL dump (frontend-editor autosave uses the same column). Module Content values live in that JSON (including Accordion/Tabs/Icon list/Gallery/Testimonial items, Video URLs, and Inner row nested columns; unknown video hosts are dropped on save; a second Inner row inside an Inner row is dropped on save; Text/CTA/Blurb/Accordion/Tabs bodies are allowlisted HTML — scripts and extra tags are stripped on save, not a separate backup field). The editor field list is application code, not a backup file. Design colors (including theme tokens and module hover), spacing (padding/margin shorthand, including four-side editor values), border/radius, allowlisted shadow / font-family / letter-spacing / text-transform keys, sticky/z-index keys, tablet/mobile overrides, section background image ids, section shape divider keys, section video background URLs, and row reverse-on-mobile are compiled or rendered from that JSON at render time; they are not a separate backup field. Background files live under uploads with the rest of Media. Public widgets (Header, After header, Homepage, Sidebar, After content, Footer) are rows in <code>cms_widgets</code> in the SQL dump; Pulse starter widgets restore with that table. Newsletter addresses restore in <code>cms_newsletter_subscribers</code> (signup flags in <code>app_settings</code>). Google Analytics / Ads / AdSense / Programmable Search IDs restore with <code>app_settings</code> (<code>google_analytics_id</code>, <code>google_ads_id</code>, <code>google_adsense_client</code>, <code>google_cse_cx</code>; Search Console stays <code>seo_google_site_verification</code>). Page/post visitor passwords restore as <code>password_hash</code> (never plaintext); the unlock-cookie HMAC key is <code>cms_content_pass_key</code> in <code>app_settings</code>.</li>
    </ul>
