...
<ul>
    ...
    <li><strong>Style packs:</strong> Upload zips into a <strong>library</strong>, then choose one from <em>Installed packs</em> and click <strong>Activate</strong>. Bundled packs (Sample template, Play · Build · Sound, <strong>Manly</strong>, <strong>Pulse</strong>) can be installed into the library. Pulse also fills empty Header / After header / Homepage / After content widget areas (occupied areas stay as they are). Clear active does not delete installed packs. WordPress theme zips map colors only — PHP templates are never executed.</li>
...
<p>Download ready-made packs from Customize / General: <strong>Sample template</strong>, <strong>Play · Build · Sound</strong>, <strong>Manly</strong> (oak / iron / leather, dark magazine), <strong>Pulse</strong> (header and homepage widgets), and <strong>Enterprise</strong> (modern navy / slate professional) — or <code>/admin/customize/sample-style-pack?pack=enterprise</code>. Rebuild with <code>php cli/build_style_pack.php</code>. Installed pack CSS lives under <code>public/uploads/theme-packs/library/</code> and is included in normal backup/restore of uploads; settings keys live in <code>app_settings</code>. Starter widgets (when present in the pack) live in <code>cms_widgets</code>.</p>
